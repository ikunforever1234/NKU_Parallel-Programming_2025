...
struct ThreadData {
    class PriorityQueue* pq;        // 指向当前PriorityQueue对象
    PT pt;                    // 要处理的PT对象
    int start_idx;            // 处理范围的起始索引
    int end_idx;              // 处理范围的结束索引
    segment* seg;             // 指向要处理的segment
    string base_guess;        // 多segment情况下的基础猜测字符串
